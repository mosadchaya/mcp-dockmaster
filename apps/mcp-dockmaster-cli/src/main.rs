...
async fn main() {
    ...
    // Handle commands
    match cli.command {
        Commands::Register { name, description, tool_type, entry_point } => {
                ...
                Ok(data) => {
                    ...
                    if let Some(servers) = data.get("servers").and_then(|s| s.as_array()) {
                        ...
                        for (i, server) in servers.iter().enumerate() {
                            println!("{}. {}", i + 1, server.get("name").and_then(|n| n.as_str()).unwrap_or("Unknown"));
                            println!("   ID: {}", server.get("id").and_then(|id| id.as_str()).unwrap_or("Unknown"));
                            println!("   Type: {}", server.get("tool_type").and_then(|t| t.as_str()).unwrap_or("Unknown"));
                            println!("   Running: {}", server.get("process_running").and_then(|p| p.as_bool()).unwrap_or(false));
                            println!("   Tool Count: {}", server.get("tool_count").and_then(|c| c.as_i64()).unwrap_or(0));
                            println!();
                        }
                    }
                    ...
                    if let Some(tools) = data.get("tools").and_then(|t| t.as_array()) {
                        ...
                        for (i, tool) in tools.iter().enumerate() {
                            println!("{}. {}", i + 1, tool.get("name").and_then(|n| n.as_str()).unwrap_or("Unknown"));
                            println!("   ID: {}", tool.get("id").and_then(|id| id.as_str()).unwrap_or("Unknown"));
                            println!("   Server: {}", tool.get("server_id").and_then(|s| s.as_str()).unwrap_or("Unknown"));
                            println!("   Proxy ID: {}", tool.get("proxy_id").and_then(|p| p.as_str()).unwrap_or("Unknown"));
                            println!("   Description: {}", tool.get("description").and_then(|d| d.as_str()).unwrap_or(""));
                            println!();
                        }
                    }
                },
        ...
        Commands::Execute { tool_id, parameters } => {
